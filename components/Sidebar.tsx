...
export default function Sidebar({ user, logout }: SidebarProps) {
  ...
  const brandColor = 'text-teal-600';
            ...
            <div className="flex flex-col ml-4 mt-1">
              {item.subLinks.map(sl => (
                <button
                  ...
                  className="px-4 py-2 text-gray-700 hover:bg-teal-50 rounded transition text-left"
                >
                  {sl.name}
                </button>
              ))}
  ...
  return (
    <>
      ...
      <div className="md:hidden flex items-center justify-between bg-white p-4 shadow">
        <div className="text-2xl font-bold text-teal-700 cursor-pointer" onClick={() => router.push('/dashboard')}>
          SwiftInvoice
        </div>
        <button onClick={() => setMobileOpen(!mobileOpen)} className="text-2xl text-gray-800">
          <FaBars />
        </button>
      </div>

      {/* Sidebar */}
      <aside className={`h-screen bg-white md:flex md:flex-col md:w-64 shadow-md p-4 absolute md:relative z-50 ${mobileOpen ? 'block' : 'hidden'} md:block`}>
        {/* Brand */}
        <div className="hidden md:block text-2xl font-bold text-teal-700 cursor-pointer mb-6" onClick={() => router.push('/dashboard')}>
          SwiftInvoice
        </div>
